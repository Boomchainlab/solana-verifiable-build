# Agu Token Go API Library

<a href="https://pkg.go.dev/github.com/stainless-sdks/agu-token-go"><img src="https://pkg.go.dev/badge/github.com/stainless-sdks/agu-token-go.svg" alt="Go Reference"></a>

The Agu Token Go library provides convenient access to the Agu Token REST API
...
```go
import (
	"github.com/stainless-sdks/agu-token-go" // imported as agutoken
)
```
...
```sh
go get -u 'github.com/stainless-sdks/agu-token-go@v0.0.1-alpha.0'
```
...
The agutoken library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`json:"...,required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `agutoken.String(string)`, `agutoken.Int(int64)`, etc.
...
```go
p := agutoken.ExampleParams{
	ID:   "id_xxx",               // required property
	Name: agutoken.String("..."), // optional property

	Point: agutoken.Point{
		X: 0,               // required field will serialize as 0
		Y: agutoken.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: agutoken.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[agutoken.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/agu-token-go/option).
...
`*agutoken.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `agutoken.File(reader io.Reader, filename string, contentType string)`
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/agu-token-go/issues) with questions, bugs, or suggestions.
